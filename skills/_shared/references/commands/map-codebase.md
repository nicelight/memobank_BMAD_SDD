...
5) Попроси у пользователя PRD delta (что хотим изменить).
   - Если delta уже оформлена как PRD — `/constitution`, если principles не ratified/partial, затем `/write-prd --delta`, `/spec-init`, `/prd`, `/spec-design` и `/prd-to-tasks FT-<NNN>`.
   - Если delta ясна, но PRD нет — сначала `/brief`, затем `/constitution`, если principles не ratified/partial, затем `/write-prd`, `/spec-init`, `/prd`, `/spec-design` и `/prd-to-tasks FT-<NNN>`.
   - Если delta сырая / направление нестабильно — сначала `/analysis` или `/brainstorm`.
   - Не переходи напрямую к `/prd-to-tasks`; для planning delta сначала нужен `/write-prd`, `/spec-init`, `/prd` и `/spec-design`.
6) Запусти `/review`.
</process>
