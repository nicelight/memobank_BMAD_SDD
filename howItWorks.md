...
| `/spec-design` | Mandatory adaptive global SDD backbone | consumes lightweight spec-index; writes backbone status, source-of-truth, architecture/boundaries/data/contracts/testing specs as needed | не создает tasks/plans/feature-local tech-specs; не раздувает T0/T1 scope | `/spec-improve FT-*` или `/spec-auto --all` |
