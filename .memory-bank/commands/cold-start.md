...
- interactive: после `/prd` всегда пройди `/spec-design`; для simple T0/T1 scope запиши minimal status с explicit `not_applicable` areas; выбери фичу, при необходимости пройди `/clarify-feature FT-<NNN>`, затем `/spec-improve FT-<NNN>`, `/prd-to-tasks FT-<NNN>` и выполняй задачи через `/execute` → `/verify` → `/red-verify` (если задача T3; optional для T2 task) → `/mb-sync`; для T2 feature completion после всех задач выполни `/red-verify --feature FT-<NNN>`
